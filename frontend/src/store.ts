...
export const useStore = create<AppState>((set, get) => {
    ...
    switch (event.type) {
      ...
      case 'playback': {
        set((s) => {
          ...
          // The like reaction lasts the whole song, not a fixed few seconds —
          // so it only clears when the room actually moves to a new track.
          if (trackChanged && meId) {
            const mine = s.roomAvatars.find((a) => a.id === meId);
            if (mine && mine.expression !== 'neutral') {
              get().socket?.send('expression', { expression: 'neutral' });
              patch.roomAvatars = s.roomAvatars.map((a) =>
                a.id === meId ? { ...a, expression: 'neutral' } : a,
              );
            }
          }
          return patch;
  ...
  return {
    ...
    triggerLikeEffect: () => {
      ...
      // No timeout here on purpose: it reverts to neutral in the 'playback'
      // handler above, the moment the room actually moves to a new track —
    ...
    loadFavorites: async () => {
      try {
        const data = await feedbackApi.saved();
        set({ favorites: data.results || [], favoritesLoaded: true });
      } catch {
        set({ favoritesLoaded: true });
      }
    },

    addFavoriteToQueue: (track) => {
      get().socket?.send('queue_add', {
